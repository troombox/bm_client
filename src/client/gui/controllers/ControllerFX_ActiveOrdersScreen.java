...
public class ControllerFX_ActiveOrdersScreen implements IClientFxController, Initializable {
	...
	ArrayList<Order> ordersWait = new ArrayList<Order>();
	ArrayList<Order> ordersInKit = new ArrayList<Order>();
	ArrayList<Order> ordersReady = new ArrayList<Order>();
    ...
    @FXML
    private Pane expectedArrivalPane;
    
    @FXML
    private TextField expectedArrivalTimeInput;
...
//    	errorMsg.setVisible(false);
		...
		String mail=null;
		waitForApprovalTable.getItems().remove(currentOrder);
		for(Order item: ordersWait) {
			if(item.getOrderID() == currentOrder.getOrderID()) {
				mail = item.getUserEmail();
			}
		}
		SendEmail sendEmail = new SendEmail(mail,
				"Your order approved! we will let you know when it is ready. Your Order Number is: " + currentOrder.getOrderID() +
				". Don't lose this number, you'll need it to approve the arrival of your order. Thanks.");
		sendEmail.main(null);
    ...
    void orderReady(ActionEvent event) {
    	...
    	//check for arrival time input:
    	int minutes = 0;
		if(!currentOrder.getTypeOfOrder().equals(OrderType.PICKUP.toString())) {
			String mintutesText = expectedArrivalTimeInput.getText();
			try {
				minutes = Integer.parseInt(mintutesText);
			} catch (NumberFormatException e) {
				errorMsg2.setVisible(true);
				errorMsg2.setText("invalid number of min's 'till arrival!");
				return;
			}
			if(minutes <= 0 ) {
				errorMsg2.setVisible(true);
		...
		populateReadyTable();
		String mailText = "Your order is ready! ";
		if(!currentOrder.getTypeOfOrder().equals(OrderType.PICKUP.toString())) {
			mailText += "It will show up in aproximately " + String.valueOf(minutes) + " minutes";
		} else {
			mailText += "soon it will be with you.";
		}
		SendEmail sendEmail = new SendEmail(currentOrder.getUserEmail(),
				mailText);
		sendEmail.main(null);
