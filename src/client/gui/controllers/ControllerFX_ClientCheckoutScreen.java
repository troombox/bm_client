...
//    		sucessfullySentPane.setVisible(true);
    ...
    void doByes(ActionEvent event) {
    	...
    	BusinessClientData bcd = (BusinessClientData)ClientUI.clientLogic.getLastDataRecieved();
	...
	public void initialize(URL location, ResourceBundle resources) {
 	...
 	public void setRefundsValue(){
    	...
    	if(ClientUI.clientLogic.getTypeOfLastDataRecieved() != DataType.SINGLE_TEXT_STRING && ClientUI.clientLogic.getTypeOfLastRequestRecieved() == RequestType.SERVER_MESSAGE_TO_CLIENT_REFUND_AMOUNT) {
    		System.out.println("ControllerFX_ClientCheckout: setRefundsValue() - last data is not CRF");
    		//something went *very* wrong!
    		return;
    	}
    	String refundsValue = (String)ClientUI.clientLogic.getLastDataRecieved();
    	labelRefundsValue.setText(refundsValue);
    	refundValue = Integer.valueOf(refundsValue);
	...
	private void setPrices() {
		...
		if (deliveryType == OrderType.UNKNOWN) {
			labelFinalPrice.setText(String.valueOf(price - refundValue));
			...
			labelFinalPrice.setText(String.valueOf(Math.round((price + deliveryPrice * numberOfRes)*0.9 - refundValue)));
			finalPrice = (int) Math.round((price + deliveryPrice * numberOfRes) *0.9 - refundValue);
		}else if(deliveryType == OrderType.DELIVERY_REGULAR){
			labelDelivery.setText(String.valueOf(deliveryPrice * numberOfRes));
			labelFinalPrice.setText(String.valueOf(price + deliveryPrice * numberOfRes - refundValue));
			finalPrice = price + deliveryPrice * numberOfRes - refundValue;
		}else {
			labelDelivery.setText("0");
			labelFinalPrice.setText(String.valueOf(price - refundValue));
			finalPrice = price - refundValue;
		}
	}
	...
	private void doSendOrderToServer(boolean isPrivate) {
    	...
    	for(Order order : ordersToSend) {
    		order.setOrderPrice(order.getOrderPrice() - refundValue);
